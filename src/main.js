...
const dataUrl = (path) => `${DATA_BASE}${path.replace(/^\//, "")}`;
...
function renderMethod(){
  const k=els.method.value || "emmanuel_wage";
  const m=METHODS[k] || METHODS.emmanuel_wage;
  els.methodTitle.textContent=m.name;
  els.methodEnglish.textContent=m.english;
  els.methodFormula.textContent=m.formula;
  els.methodNeeds.textContent="Data required: "+m.needs;
...
async function init(){
  try{
    const [topology, metadata] = await Promise.all([
      fetch("https://cdn.jsdelivr.net/npm/world-atlas@2/countries-50m.json").then(r=>{if(!r.ok) throw new Error("World map failed to load"); return r.json();}),
      fetch(dataUrl("data/meta.json")).then(r=>{if(!r.ok) throw new Error("data/meta.json was not found"); return r.json();})
    ]);

    meta=metadata || {};
    world=feature(topology,topology.objects.countries).features;

    // Populate controls only after metadata exists. The old build rendered the
    // methodology before this step, which could crash the whole application.
    populateMethods();
    applyAvailability();

    const available=meta.available_methods || [];
    if(!available.length || !(meta.years||[]).length){
      els.sourceBadge.textContent="Real data not built yet";
      els.loading.textContent="The website is working, but no calculated dataset has been published yet. Run the ‘Refresh open unequal-exchange data’ GitHub Action, then this map will populate automatically.";
      els.method.disabled=true;
    ...
    if(available.includes("emmanuel_wage")) els.method.value="emmanuel_wage";
    else els.method.value=available.find(k=>METHODS[k]) || available[0];

    rebuildYearsForMethod();
    renderMethod();
    els.sourceBadge.textContent="Real source data · model-derived estimates";
    els.loading.remove();
    await loadYear();
  }catch(e){
    console.error(e);
    els.sourceBadge.textContent="Load error";
    els.loading.textContent="Could not start the atlas: "+e.message;
  }
}

async function loadYear(){
  selected=null;
  const method=els.method.value||"emmanuel_wage", year=els.year.value||methodYears().at(-1);
  let payload;
  try{
    const r=await fetch(dataUrl(`data/${method}-${year}.json`));
...
function render(){
  ...
  const legendUnit=metric==="gdp_share"?"pct":metricUnit();
  els.legendScale.textContent=`${fmt(-maxAbs,legendUnit)}   •   0   •   ${fmt(maxAbs,legendUnit)}`;
...
function drawLine(el,data,xv,yv,opt={}){
  ...
  svg.append("g").attr("transform",`translate(${m.l},0)`).call(d3.axisLeft(y).ticks(5).tickFormat(v=>(opt.unit==="hours"||opt.unit==="persons")?d3.format(".2s")(v):"$"+d3.format(".2s")(v))).call(g=>g.selectAll("text").attr("fill","#9aa8b6")).call(g=>g.selectAll("line,path").attr("stroke","#354352"));
